...
//      TF1* sumFunc = new TF1("sumFunc","fnFunc + fnFunc2");
      ...
      sumFunc->SetParameter(0,14000/6. * _time);
      sumFunc->SetParameter(1,intercept );
      sumFunc->SetParameter(2,0.65);
      sumFunc->SetParameter(3,14000/6. * _time);
      sumFunc->SetParameter(4,intercept );
      sumFunc->SetParameter(5,2);
 
         Int_t icounter = 0, paddle =0, nEvent=0;
         TRandom3* r3 = new TRandom3();
         Double_t fillingEvt[1000000]={};
         for(Int_t iBin=0; iBin<nBins ; ++iBin) {
	Double_t content = sumFunc ->Integral(1+iBin*7./nBins, 1+(iBin+1)*7./nBins);
	junoSpec[iBin]=content;
	nEvent = content + 0.0000001 ;
          for(Int_t iEvent = 0; iEvent < nEvent; iEvent ++) {
              r3->SetSeed(iBin*100+iEvent*4);
              fillingEvt[paddle + iEvent] = r3->Gaus(0,1./(double)nBins) + allVec[0]->GetBinCenter(iBin+1);
             icounter ++;
                 }
             paddle += nEvent;
         }
...
*/
   ...
   for(Int_t i=0;i<icounter ;i++){   
         ...
         Double_t thisL = ((RooAbsReal*)_pulls->at(9))->getVal() * 52.5;
         Double_t weight = this->surv_JUNO( (thisE+0.8) / 1000. , _pulls, thisL) ;
          allVec[1]->Fill(thisE ,weight);
                }

    cout<<allVec[1]->GetBinContent(10)<<" "<<allVec[1]->GetBinContent(100)<<endl;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 //  std::cout<<"setting up data "<<std::endl;
   ...
   for(Int_t i=0;i<nBins;i++){ 
        ...
        (*fData_JUNO)[i] = junoSpec[i] * this->surv_JUNO( (thisE2+0.8) / 1000. , dataIn, 52.5 )  ;  
...
vecInput2 ->SetBinContent(8,0.03);
vecInput2 ->SetBinContent(9,0.03);
vecInput2 ->SetBinContent(10,0.004); 
