...
for(Int_t i=0;i<11;i++){std::cout<<" "<<rep->getPar(i)<<std::endl;}
    ...
    double bin_x[npts ]={};
    double bin_y[npts ]={};
    double low_x = 0, high_x = 1.2, low_y = 2.e-3, high_y = 3.2e-3;
    double step_x = (high_x-low_x)/(double)npts; double step_y = (high_y - low_y)/(double)npts;   
    for(int i=0;i<npts ;i++){bin_x[i]= low_x + i * step_x;}
    for(int i=0;i<npts ;i++){bin_y[i]= low_y + i * step_y;}
    ...
    rep ->getParVar(1)->setVal(bin_x[i]);
    rep ->getParVar(5)->setVal(bin_y[j]); 
	...
	deltascany[i] = scany[i] - bestFit ;
      ...
      out1<<i<<" "<<bin_x[i]<<" "<<bin_y[j]<<" "<<deltascany[i]<<std::endl;
         }
    }
