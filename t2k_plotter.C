...
ifstream in9;
double min=800;
in.open("./t2k1D_s23.txt");
in2.open("./t2kSk1D_s23.txt");
in4.open("./t2k2D_s23.txt");
in5.open("./t2kSk2D_s23.txt");
...
TH2D* p1_CL = new TH2D("p1_CL","",50,0.0,1.2,   120, 2,3.2);
TH2D* p1_CL2 = new TH2D("p1_CL2","",50,0.0,1.2,   120, 2,3.2);
...
{break;}
if (e<minn1 ){minn1=e;}
int shere=((c+0.00000001) - 0 )/(1.2/50.);
int ahere=((d+0.00000001) - 2e-3)/(1.2e-3/120.);
min1[shere][ahere]=e;
}
in4.close();
...
double xx[50]={};
double xx1[50]={};
for(int i=0;i<50;i++){xx[i] = 0. + i*0.0314; xx1[i] = 0. + i*0.0314; }
...
TMultiGraph *mg = new TMultiGraph();
 ...
 // mg->SetMaximum(0.04);
mg->Draw("ALP");
mg->GetXaxis()->SetTitle("#theta_{23}");
...
cout<<"done in1 "<<minn1<<" "<<minn2<<endl;
   ...
   double conto3[1]={minn1+6.8};
   double conto4[1]={minn2+6.8};
...
p1_CL2->GetXaxis()->SetTitle("#theta_{23}");// sin^{2}(2 #theta_{13})");
p1_CL2->GetYaxis()->SetTitle("#Delta m^{2} [10^{-3}]");
