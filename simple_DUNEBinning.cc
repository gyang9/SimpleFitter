...
TMatrixD* Sterile::prepareT2kCovMatrixND(TMatrixD* covM_t2k, TVectorD* fVec_t2k, TVectorD* fVec_t2k_nd, Int_t nBins) const
{
Int_t nbin = nBins*3;
TMatrixD* covmatT2k = new TMatrixD(nbin,nbin);
TVectorD* fVec_t2k_tot = new TVectorD(nbin);

    for(Int_t i=0;i<nbin/2;i++){
      (*fVec_t2k_tot)[i] = (*fVec_t2k)[i];
                             }
    for(Int_t i=0;i<nbin/2;i++){
      (*fVec_t2k_tot)[i+nbin/2] = (*fVec_t2k_nd)[i];
                             }

    for(Int_t i=0;i<nbin;i++){
      for(Int_t j=0;j<nbin; j++){
        (*covmatT2k )(i,j) = (*covM_t2k)(i,j) * (*fVec_t2k_tot)[i] * (*fVec_t2k_tot)[j];
                                }
                             }  
    for(Int_t i=0;i<nbin;i++){
          (*covmatT2k )(i,i) += (*fVec_t2k_tot)[i];                           
                             }
std::cout<<"t2k matrix sum "<<covmatT2k->Sum()<<std::endl;
return covmatT2k ;
}
...
Double_t Sterile ::FillEv( RooListProxy* _pulls ) const
{
   ...
   Double_t numuVec_t2k_nd[nBins];
   Double_t nueVec_t2k_nd[nBins];
 ...
 //+ [6] * TMath::Erf((x-0.8)*2) ",0.,endPoint );
      ...
      TFile f1("g4lbne_FHC_FD.root");
      TH1* numuVec_fd_file = (TH1*)f1.Get("numu_flux");
      TH1* nueVec_fd_file = (TH1*)f1.Get("nue_flux");

      TFile f2("g4lbne_FHC_ND.root");
      TH1* numuVec_nd_file = (TH1*)f2.Get("numu_flux");
      TH1* nueVec_nd_file = (TH1*)f2.Get("nue_flux");


        Double_t scaling = 10e14 * _time;

        for(Int_t iBin=0; iBin<nBins ; ++iBin) {
	Double_t content = numuVec_fd_file ->Integral(iBin*(endPoint - intPoint)/nBins, (iBin+1)*(endPoint - intPoint)/nBins);
	numuVec_t2k[iBin]=content*scaling;
	Double_t content2 = nueVec_fd_file ->Integral(iBin*(endPoint - intPoint)/nBins, (iBin+1)*(endPoint - intPoint)/nBins);
	nueVec_t2k[iBin]=content2*scaling;

	Double_t content3 = numuVec_nd_file ->Integral(iBin*(endPoint - intPoint)/nBins, (iBin+1)*(endPoint - intPoint)/nBins);
	numuVec_t2k_nd[iBin]=content3*scaling;
	Double_t content4 = nueVec_nd_file ->Integral(iBin*(endPoint - intPoint)/nBins, (iBin+1)*(endPoint - intPoint)/nBins);
	nueVec_t2k_nd[iBin]=content4*scaling;
         }   
...
*/   
   ...
   for(Int_t i=0;i<nBins*2;i++){
      for(Int_t j=0;j<nBins*2;j++){
          if(i!=j) (*covM_t2k_nd)(i,j) = nCorr1  * TMath::Sqrt((*covM_t2k_nd)(i,i)*(*covM_t2k_nd)(j,j)) ;
       }
    }
   ...
   for(Int_t i=0;i<nBins ;i++){
      for(Int_t j=nBins ;j<nBins *2;j++){
        (*covM_t2k_nd)(i,j) = nCorr3  * TMath::Sqrt((*covM_t2k_nd)(i,j-nBins )*(*covM_t2k_nd)(i+nBins ,j)) ;
   ...
   TMatrixD* covM_t2k_tot = new TMatrixD(nBins+nBins+nBins,nBins+nBins+nBins);
      ...
      for(Int_t i=0;i<nBins*1.5 ;i++){
      for(Int_t j=0 ;j<nBins*1.5 ;j++){
        (*covM_t2k_tot)(i + nBins * 1.5, j + nBins * 1.5) = (*covM_t2k_nd)(i,j) ;
       }
    }

   for(Int_t i=0;i<nBins*1.5 ;i++){
      for(Int_t j=nBins*1.5 ;j<nBins *3;j++){
        (*covM_t2k_tot)(i,j) = nCorr4  * TMath::Sqrt((*covM_t2k_tot)(i,j-nBins*1.5 )*(*covM_t2k_tot)(i+nBins*1.5 ,j)) ;
   ...
   allVec[2] = new TH1D("","",nBins ,0.,endPoint );
   allVec[3] = new TH1D("","",nBins /2,0.,endPoint/2);
   ...
   for(Int_t i=0;i<nBins ;i++){ (*fVec_t2k)[i] = numuVec_t2k[i] * 
        this->surv_t2k(1300, allVec[0]->GetBinCenter(i+1) * ((RooAbsReal*)_pulls->at(7))->getVal(), _pulls)  * ((RooAbsReal*)_pulls->at(9))->getVal() ; 
                      }
   for(Int_t i=nBins ;i<nBins *1.5;i++){ (*fVec_t2k)[i] = numuVec_t2k[i-nBins ] * this->app_t2k(1300, allVec[1]->GetBinCenter(i+1-nBins ) * ((RooAbsReal*)_pulls->at(8))->getVal() , _Density,  _pulls) * ((RooAbsReal*)_pulls->at(10))->getVal() ; 
        (*fVec_t2kShadow)[i-nBins] = (*fVec_t2k)[i];
                      }



   TVectorD* fVec_t2k_nd = new TVectorD(nBins *1.5);
   TVectorD* fVec_t2kShadow_nd = new TVectorD(nBins);

   for(Int_t i=0;i<nBins ;i++){ (*fVec_t2k_nd)[i] = numuVec_t2k_nd[i] * 
        this->surv_t2k(0.5, allVec[2]->GetBinCenter(i+1) * ((RooAbsReal*)_pulls->at(9))->getVal() , _pulls) * ((RooAbsReal*)_pulls->at(7))->getVal()  ; 
                      }
   for(Int_t i=nBins ;i<nBins *1.5;i++){ (*fVec_t2k_nd)[i] = numuVec_t2k_nd[i-nBins ] * this->app_t2k(0.5, allVec[3]->GetBinCenter(i+1-nBins ) * ((RooAbsReal*)_pulls->at(10))->getVal() , _Density,  _pulls) * ((RooAbsReal*)_pulls->at(8))->getVal()  ; 
        (*fVec_t2kShadow_nd)[i-nBins] = (*fVec_t2k_nd)[i];
                      }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 //  std::cout<<"setting up data "<<std::endl;
   TVectorD* fData_t2k = new TVectorD(nBins *3);
   ...
   for(Int_t i=0;i<nBins ;i++){ (*fData_t2k)[i+nBins*1.5] = numuVec_t2k_nd[i] * this->surv_t2k(0.5, allVec[2]->GetBinCenter(i+1) , dataIn )  ;  }
   for(Int_t i=nBins ;i<nBins *1.5;i++){ (*fData_t2k)[i+nBins*1.5] = numuVec_t2k_nd[i-nBins ] * this->app_t2k(0.5, allVec[3]->GetBinCenter(i+1-nBins ), _Density , dataIn ) ; }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   TMatrixD* covMatT2k = this->prepareT2kCovMatrixND(covM_t2k_tot, fVec_t2k, fVec_t2k_nd, nBins);

 //  fVec_JUNO->Print();
 //  std::cout<<"Ok"<<std::endl;
 //  fData_JUNO->Print();

  TVectorD* fVec_t2k_tot = new TVectorD(nBins *3);

    for(Int_t i=0;i<nBins*1.5;i++){
      (*fVec_t2k_tot)[i] = (*fVec_t2k)[i];
                             }
    for(Int_t i=0;i<nBins*1.5;i++){
      (*fVec_t2k_tot)[i+nBins*1.5] = (*fVec_t2k_nd)[i];
                             }
   
   for(Int_t i=0;i<nBins *3; i++){
       (*fVec_t2k_tot)[i] -= (*fData_t2k)[i];
         if( (*covMatT2k)(i,i) ==0 ){(*covMatT2k)(i,i) = 1000000000;}
     }

   TFile* outFile = TFile::Open("matStore.root", "RECREATE");
   covMatT2k->Write("mat");
   outFile -> Write();
 ...
 //   fVec_t2k->Print();
   ...
   TVectorD mulVec(*fVec_t2k_tot);
   mulVec *= (*covMatT2k);

   Double_t t2kResult = TMath::Abs(mulVec*(*fVec_t2k_tot));
...
vecInput2 ->SetBinContent(9,0.05);
vecInput2 ->SetBinContent(10,0.05); 
vecInput2 ->SetBinContent(11,0.05);
...
rep->addSK(true);
  ...
  Int_t binSetup = 20;
  Double_t timeSetup = 1;

  Double_t corr1Setup = atof(argv[2]);
  Double_t corr2Setup = atof(argv[3]);
  Double_t corr3Setup = atof(argv[4]);
  Double_t corr4Setup = atof(argv[5]);

//  Double_t corr1Setup = 0.5;
//  Double_t corr2Setup = 0.12;
//  Double_t corr3Setup = 0.12;
//  Double_t corr4Setup = 0.6;

rep->setNBins(binSetup);
//rep->setNBins(atof(argv[2]));

rep->setCorr1(corr1Setup);
rep->setCorr2(corr2Setup);
rep->setCorr3(corr3Setup);
rep->setCorr4(corr4Setup);

rep->setTime(timeSetup);
...
rep->FillEv(rep->getPullList());
  ...
  int aa1 = (atof(argv[2])+0.000001)*100;
  int aa2 = (atof(argv[3])+0.000001)*100;
  int aa3 = (atof(argv[4])+0.000001)*100;
  int aa4 = (atof(argv[5])+0.000001)*100;

  ofstream out2;
  out2.open(Form("s%d_a%d_b%d_c%d_DUNE.txt",aa1,aa2,aa3,aa4 ),ios::app);
...
for(Int_t i=0;i<11;i++){std::cout<<" "<<rep->getPar(i)<<std::endl;}
  ...
  out2<<aa1 <<" "<<aa2<<" "<<aa3<<" "<<aa4<<" "<<bb<<" "<<dd<<" "<<bestFit<<endl; 
  cout<<aa1 <<" "<<aa2<<" "<<aa3<<" "<<aa4<<" "<<bb<<" "<<dd<<" "<<bestFit<<endl; 
