...
Double_t Sterile ::FillEv( RooListProxy* _pulls ) const
{
   ...
   Double_t nueVec_t2k[nBins];
      ...
      TF1* sumFunc = new TF1("sumFunc","[0]*exp(-0.5 * TMath::Power((x-[1])/[2],2)) + [3]*exp(-0.5 * TMath::Power((x-[4])/[5],2)) + [6] * TMath::Erf((x-0.8)*2) ",0.,endPoint );

      sumFunc->SetParameter(0,380 * _time);
      sumFunc->SetParameter(1,intercept );
      sumFunc->SetParameter(2,0.18);
      sumFunc->SetParameter(3,530 * _time);
      sumFunc->SetParameter(4,intercept );
      sumFunc->SetParameter(5,0.2);
   ...
   for(Int_t i=0;i<nBins ;i++){
      for(Int_t j=nBins ;j<nBins *2;j++){
        (*covM_t2k)(i,j) = 0.12 * TMath::Sqrt((*covM_t2k)(i,j-nBins )*(*covM_t2k)(i+nBins ,j)) ;
   ...
   Int_t nSpec = 2;
   TH1D* allVec[nSpec];
   allVec[0] = new TH1D("","",nBins ,0.,2.);
   allVec[1] = new TH1D("","",nBins /2,0.,1.);
   ...
   for(Int_t i=0;i<nBins ;i++){ (*fVec_t2k)[i] = numuVec_t2k[i] * 
        this->surv_t2k(295, allVec[0]->GetBinCenter(i+1) , _pulls) * ((RooAbsReal*)_pulls->at(7))->getVal() * ((RooAbsReal*)_pulls->at(9))->getVal() ; 
           (*fVec_t2kShadow)[i] = numuVec_t2k[i];
                      }
   for(Int_t i=nBins ;i<nBins *1.5;i++){ (*fVec_t2k)[i] = numuVec_t2k[i-nBins ] * this->app_t2k(295, allVec[1]->GetBinCenter(i+1-nBins ) , _Density,  _pulls) * ((RooAbsReal*)_pulls->at(8))->getVal() * ((RooAbsReal*)_pulls->at(10))->getVal() ; }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 //  std::cout<<"setting up data "<<std::endl;
   ...
   for(Int_t i=0;i<nBins ;i++){ (*fData_t2k)[i] = numuVec_t2k[i] * this->surv_t2k(295, allVec[0]->GetBinCenter(i+1) , dataIn )  ;  }
   for(Int_t i=nBins ;i<nBins *1.5;i++){ (*fData_t2k)[i] = numuVec_t2k[i-nBins ] * this->app_t2k(295, allVec[1]->GetBinCenter(i+1-nBins ), _Density , dataIn ) ; }



TH1D* hShadow_t2k  =  new TH1D("","",nBins,0,2);
TH1D* hData_t2k  = new TH1D("","",nBins,0,2);
for(Int_t i=0;i<nBins; i++){
hShadow_t2k  ->SetBinContent(i+1,(*fVec_t2kShadow  )(i));
hData_t2k  ->SetBinContent(i+1,(*fVec_t2k  )(i));
}

  ofstream out;
  out.open(Form("bin%d.txt",nBins),ios::out);
     for(Int_t i=0;i<nBins;i++){ out<<hShadow_t2k  ->GetBinContent(i+1)<<" "<<hData_t2k  ->GetBinContent(i+1)<<endl; }
...
for(Int_t i=0;i<11;i++){std::cout<<" "<<rep->getPar(i)<<std::endl;}
  ...
  out2<<atoi(argv[2])<<" "<<aa<<" "<<bb<<" "<<dd<<endl; 
  cout<<atoi(argv[2])<<" "<<aa<<" "<<bb<<" "<<dd<<endl; 
