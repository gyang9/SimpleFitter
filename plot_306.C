...
for(Int_t i=2;i<p2->GetNbinsX();i++) { 
  for (Int_t j=2;j<p2->GetNbinsY();j++){
       if(TMath::Abs(p2->GetBinContent(i,j)-p2->GetBinContent(i+1,j))>2 && TMath::Abs(p2->GetBinContent(i,j)-p2->GetBinContent(i-1,j)) >2 ) p2->SetBinContent(i,j,(p2->GetBinContent(i+1,j)+p2->GetBinContent(i-1,j))/2.);
     }
  }
for(Int_t i=2;i<p2->GetNbinsX();i++) { 
  for (Int_t j=2;j<p2->GetNbinsY();j++){
       if(TMath::Abs(p2->GetBinContent(i,j)-p2->GetBinContent(i,j+1))>2 && TMath::Abs(p2->GetBinContent(i,j)-p2->GetBinContent(i,j-1)) >2 ) p2->SetBinContent(i,j,(p2->GetBinContent(i,j+1)+p2->GetBinContent(i,j-1))/2.);
     }
  }

for(Int_t i=4;i<p2->GetNbinsX()-3;i++) { 
  for (Int_t j=4;j<p2->GetNbinsY()-3;j++){
        if(p2->GetBinContent(i,j)>5|| p2->GetBinContent(i,j)<0.05)  p2->SetBinContent(i,j, (p2->GetBinContent(i+3,j+3)+p2->GetBinContent(i-3,j-3))/2.);
     }
  }
for(Int_t i=4;i<p2->GetNbinsX()-3;i++) { 
  for (Int_t j=4;j<p2->GetNbinsY()-3;j++){
        if(p2->GetBinContent(i,j)>5|| p2->GetBinContent(i,j)<0.05)  p2->SetBinContent(i,j, (p2->GetBinContent(i+6,j+6)+p2->GetBinContent(i-6,j-6))/2.);
     }
  }
for(Int_t i=4;i<p2->GetNbinsX()-3;i++) { 
  for (Int_t j=4;j<p2->GetNbinsY()-3;j++){
        if(p2->GetBinContent(i,j)>5|| p2->GetBinContent(i,j)<0.05)  p2->SetBinContent(i,j, (p2->GetBinContent(i+7,j+7)+p2->GetBinContent(i-7,j-7))/2.);
     }
  }

for(Int_t i=3;i<p2->GetNbinsX()-1;i++) { 
  for (Int_t j=3;j<p2->GetNbinsY()-1;j++){
       if(TMath::Abs(p2->GetBinContent(i,j)-p2->GetBinContent(i+2,j))>2 && TMath::Abs(p2->GetBinContent(i,j)-p2->GetBinContent(i-2,j)) >2 ) p2->SetBinContent(i,j,(p2->GetBinContent(i+2,j)+p2->GetBinContent(i-2,j))/2.);
     }
  }
for(Int_t i=3;i<p2->GetNbinsX()-1;i++) { 
  for (Int_t j=3;j<p2->GetNbinsY()-1;j++){
       if(TMath::Abs(p2->GetBinContent(i,j)-p2->GetBinContent(i,j+2))>2 && TMath::Abs(p2->GetBinContent(i,j)-p2->GetBinContent(i,j-2)) >2 ) p2->SetBinContent(i,j,(p2->GetBinContent(i,j+2)+p2->GetBinContent(i,j-2))/2.);
     }
  }

for(Int_t i=6;i<p2->GetNbinsX()-6;i++) { 
  for (Int_t j=6;j<p2->GetNbinsY()-6;j++){
       if(TMath::Abs(p2->GetBinContent(i,j)-p2->GetBinContent(i+6,j))>2 && TMath::Abs(p2->GetBinContent(i,j)-p2->GetBinContent(i-6,j)) >2 ) p2->SetBinContent(i,j,(p2->GetBinContent(i+6,j)+p2->GetBinContent(i-6,j))/2.);
     }
  }
for(Int_t i=6;i<p2->GetNbinsX()-6;i++) { 
  for (Int_t j=6;j<p2->GetNbinsY()-6;j++){
       if(TMath::Abs(p2->GetBinContent(i,j)-p2->GetBinContent(i,j+6))>2 && TMath::Abs(p2->GetBinContent(i,j)-p2->GetBinContent(i,j-6)) >2 ) p2->SetBinContent(i,j,(p2->GetBinContent(i,j+6)+p2->GetBinContent(i,j-6))/2.);
     }
  }
